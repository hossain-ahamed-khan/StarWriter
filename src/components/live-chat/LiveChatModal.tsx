...
function LiveChatModal() {
  ...
  const handleSendMessage = async () => {
    ...
    // Mark as interacted (removes welcome message on first send)
    if (!hasInteracted) {
    ...
    try {
      ...
      console.log('Live chat response:', response);

      // Extract data from response
      const { thread_id, user_message, bot_message } = response.data;

      // Update thread ID if it's the first message
    ...
    <>
      ...
      {!isOpen && (
        <button
          ...
          className="w-14 h-14 rounded-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 shadow-2xl flex items-center justify-center transition-all duration-300 hover:scale-110 group"
