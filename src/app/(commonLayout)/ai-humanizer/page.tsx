...
const AiHumanizer = () => {
    ...
    const getProgressMessage = () => {
        if (progress < 25) return 'Analyzing text...';
        if (progress < 50) return 'Humanizing content...';
        if (progress < 75) return 'Refining output...';
        return 'Almost done...';
    };
    ...
    const getErrorTitle = (type: string) => {
        switch (type) {
            ...
            case 'payment': return 'Insufficient Credits';
            case 'timeout': return 'Request Timeout';
            case 'connection':
            case 'network': return 'Connection Error';
            case 'server': return 'Server Error';
            case 'forbidden': return 'Access Denied';
            case 'not_found': return 'Account Setup Required';
